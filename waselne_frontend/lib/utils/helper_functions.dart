...
Future<bool> saveToken(String token) async {
  ...
  //access token is the key
  //so i will set the token with the key
}

//this will get the token from the key
Future<String> getToken() async {
  final pref = await SharedPreferences.getInstance();
  return pref.getString(accessToken) ?? '';
  //if the access token is not set
  //or if there is no access token
}

Future<bool> saveLoginTime(int time) async {
  final pref = await SharedPreferences.getInstance();
  return pref.setInt(loginTime, time);
  //the key is the loginTime
  //these are the time of data that you can save in the shared
  //reference file
}
